...
class BlogPostModelTest(TestCase):
    def setUp(self):
        ...
        self.test_post = BlogPosts.objects.create(
            title="test_title", content="test_content", author=test_user
        )
...
class BlogPostListTest(TestCase):
    def setUp(self):
        ...
        self.blog_post = BlogPosts.objects.create(
            title="Test Post", content="Test content", author=self.test_user
        )
...
class BlogPostDetailTest(TestCase):
    ...
    def test_view_returns_200_for_existing_post(self):
        response = self.client.get(
            reverse("post-detail", kwargs={"pk": self.blog_post.pk})
        )
